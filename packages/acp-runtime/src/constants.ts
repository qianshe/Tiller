...
export const DEFAULT_ACP_SESSION_REQUEST_TIMEOUT_MS = 120_000;
export const DEFAULT_ACP_PROMPT_TIMEOUT_MS = 30 * 60_000;
export const DEFAULT_ACP_PROMPT_START_TIMEOUT_MS = 2 * 60_000;
...
export function resolveAcpRequestTimeout(
  ...
  if (isAcpSessionRequest(method)) {
    return resolveAdapterRequestTimeout(provider, method)
...
function isAcpSessionRequest(method: string): boolean {
