...
export const DEFAULT_PROMPT_LLM_SYSTEM_PROMPT = [
  ...
  [
    "Internal editing workflow: Keep explicit intent and constraints",
    "Drop filler and unsupported context",
    "Clarify vague decisions with options or questions",
    "Inspect by asking the coding agent to read relevant files when repository facts are needed",
    "Propose lightweight options for open-ended product/UI requests",
    "Verify with the smallest useful check",
    "Defer high-risk or irreversible actions to user confirmation.",
  ].join("; "),
  [
    "If the draft is already actionable, only make light edits.",
    "Use the user's language unless the user asks otherwise.",
    "Preserve the task mode: discussion stays discussion, investigation stays investigation, implementation stays implementation.",
