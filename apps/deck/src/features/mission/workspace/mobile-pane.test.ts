...
test("MISSION_MOBILE_PANE_ORDER keeps the mission mobile navigation order", () => {
  assert.deepEqual(MISSION_MOBILE_PANE_ORDER, ["project", "chat", "inspector", "display"]);
});
...
test("selectAdjacentMissionMobilePane moves within pane bounds", () => {
  ...
  assert.equal(selectAdjacentMissionMobilePane("chat", 1), "inspector");
  assert.equal(selectAdjacentMissionMobilePane("inspector", 1), "display");
  assert.equal(selectAdjacentMissionMobilePane("display", 1), "display");
});
