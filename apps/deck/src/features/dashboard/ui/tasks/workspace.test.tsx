...
test("DashboardTaskWorkspace renders only the selected task view", () => {
  const panelHtml = renderToStaticMarkup(
    createElement(DashboardTaskWorkspace, { sessions }),
  );
  ...
  assert.match(panelHtml, /data-task-column="archived"/);
  assert.match(panelHtml, /已归档/);
...
test("task workspace keeps the board mapping close to the view contract", () => {
  ...
  assert.equal(resolveTaskBoardColumn(sessions[1]!), "ready");
  ...
  assert.match(workspaceSource, /onDragEnd/);
  assert.equal(isArchivableTask({ agentName: "Codex", status: "idle" }), true);
  assert.equal(isArchivableTask({ agentName: "Codex", status: "error" }), true);
  assert.equal(isArchivableTask({ agentName: "Codex", status: "running" }), false);
  assert.equal(isArchivableTask({ agentName: null, status: "idle" }), false);
});
